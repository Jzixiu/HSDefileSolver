...
vector<minion> ally;
vector<minion> enemy;
vector< vector<int> > lethal_target;//每个友方随从能够同归于尽的目标
...
void kill(int ally_id)
{
    ...
    kill(ally_id+1);//不与任何敌方随从同归于尽的情况

    for(int i=0;i<lethal_target[ally_id].size();i++)
    {
        int enemy_id=lethal_target[ally_id][i];
        assert(enemy_id<m);
        if(enemy[enemy_id].hp<=0) continue;
...
void init()
{
    ...
    for(int ally_id=0;ally_id<n;ally_id++)
    {
        vector<int> temp;
        for(int enemy_id=0;enemy_id<m;enemy_id++)
        {
            if(ally[ally_id].atk >= enemy[enemy_id].hp 
                && enemy[enemy_id].atk >= ally[ally_id].hp)
            {
                temp.push_back(enemy_id);
            }
        }
        lethal_target.push_back(temp);
    }

    assert(lethal_target.size()==n);
...
int main()
{
    ...
    ofstream outfile("/Users/venoflame/desktop/code/HSDefile/res_optimal_fast_guess");
